        ...
        <div className="flex-grow flex flex-col justify-center bg-gray-50 border border-gray-200 rounded-sm p-5 relative overflow-hidden">
          ...
          <div className="flex items-end gap-3 mb-4">
            <span className="text-4xl font-black text-gray-900 leading-none">
              {data?.current_market?.currency || '$'}{data?.current_market?.currentPrice || '---'}
            </span>
            <div className="flex flex-col pb-1">
              <span className="text-sm font-bold text-gray-600">{data?.current_market?.unit || ''}</span>
              <span className={`text-sm font-black flex items-center ${data?.current_market?.trend === 'up' ? 'text-green-700' : data?.current_market?.trend === 'down' ? 'text-red-700' : 'text-gray-600'}`}>
                {data?.current_market?.trend === 'up' ? <TrendingUp className="w-4 h-4 mr-1" /> : data?.current_market?.trend === 'down' ? <TrendingDown className="w-4 h-4 mr-1" /> : null}
                {data?.current_market?.percentageChange || ''}
              </span>
            </div>
          </div>
          ...
          <div className="bg-blue-50 rounded-sm p-4 border border-blue-200 mt-2">
            <p className="text-sm text-blue-900 font-bold leading-relaxed">
              ...
              {data?.current_market?.insight || 'No market insight available at the moment.'}
            </p>
          </div>
        </div>
      ) : (
